...
private:
    ...
    int SetupStack(BindingMap<K1> *bindings1, BindingMap<K2> *bindings2, const K1 &key1, const K2 &key2, int number_of_arguments);
    ...
    template <typename T>
    void ReplaceArgument(T value, uint8 index);
    template <typename K1, typename K2>
    void CallAllFunctions(BindingMap<K1> *bindings1, BindingMap<K2> *bindings2, const K1 &key1, const K2 &key2);
    template <typename K1, typename K2>
    bool CallAllFunctionsBool(BindingMap<K1> *bindings1, BindingMap<K2> *bindings2, const K1 &key1, const K2 &key2, bool default_value = false);
    ...
    template <typename K>
    int SetupStack(BindingMap<K> *bindings, const K &key, int number_of_arguments)
    {
    ...
    template <typename K>
    bool CallAllFunctionsBool(BindingMap<K> *bindings, const K &key, bool default_value = false)
    {
    ...
    void Push(const unsigned long long value)
    {
        Push(L, value);
        ++push_counter;
    }
    ...
    void Push(ObjectGuid const value)
    {
        Push(L, value);
        ++push_counter;
    }
    void Push(const CreatureTemplate *value)
    {
        Push(L, value);
        ++push_counter;
    }
...
public:
    ...
    // Checks
    template <typename T>
    static T CHECKVAL(lua_State *luastate, int narg);
    template <typename T>
    static T CHECKVAL(lua_State *luastate, int narg, T def)
    {
        return lua_isnoneornil(luastate, narg) ? def : CHECKVAL<T>(luastate, narg);
    }
    template <typename T>
    static T *CHECKOBJ(lua_State *luastate, int narg, bool error = true)
    {
    ...
    void OnPlayerSpellDamage(Player *player, Unit *target, int32 &damage, uint32 spellId); //
...
template <>
Unit *Eluna::CHECKOBJ<Unit>(lua_State *L, int narg, bool error);
template <>
Object *Eluna::CHECKOBJ<Object>(lua_State *L, int narg, bool error);
template <>
WorldObject *Eluna::CHECKOBJ<WorldObject>(lua_State *L, int narg, bool error);
template <>
ElunaObject *Eluna::CHECKOBJ<ElunaObject>(lua_State *L, int narg, bool error);
